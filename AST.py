...
class Gramatica:
                        ...
                        if (item[d][1] == nodo.izq):
                            ...
                            if (d < len(item)-1 and item[d+1][0] == 1):                          
                                ...
                                try:
                                    ...
                                    self.siguientes_d[nodo.izq] = [item[d+1][1]]
                                existe = True
                                break
            ...
            if (not existe):
                ...
                if (index_padre == ''):
                    ...
                    try:    
                        ...
                        #Posible problema
                        #self.siguientes_d[index_padre] = self.siguientes_d[nodo.izq]
                        self.siguientes_d[index_padre] = ['$']
    ...
    def Analiza(self, cadena : [], arbol: Arbol):
        ...
        try:
            while( len(entrada) != 0): # and len(pila) != 0):
                if (entrada[0][0] == pila[0]):
                    ...
                    tmp = pila.popleft()
                    #Ubicacion de pivote
                    #if (tmp == '$' and len(entrada) != 0):
                    #    continue
                    
                    for x in reversed(self.tabla[tmp,entrada[0][0]]):
                        if (x != 'lambda'):                  
                            pila.appendleft(x)
                            #Opera1
                            #Crear Nuevo Nodo e Insertar como hijo
                            if (x == 'Num'):
                                hijo = Nodo(x, pivote, value = entrada[0][1])      
                            elif (x == 'Var'):
                                if (entrada[0][0] == 'int' or
                                    entrada[0][0] == 'node' or
                                    entrada[0][0] == 'edge' or
                                    entrada[0][0] == 'flecha'):
                                    hijo = Nodo(x, pivote, value = entrada[1][1]) #Mejorar   
                                else:                                    
                                    hijo = Nodo(x, pivote, value = entrada[0][1])      
                            else:
                                hijo = Nodo(x, pivote, linea=entrada[0][2], col=entrada[0][3])
